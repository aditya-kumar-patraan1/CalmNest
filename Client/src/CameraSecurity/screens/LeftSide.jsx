...
export const LeftSide = ({ room, email }) => {
      ...
      return (
          <AnimatePresence>
              <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="fixed inset-0 z-50 bg-[#FCFDFD] backdrop-blur-sm flex items-center justify-center p-4"
              >
                  <motion.div
                      initial={{ scale: 0.8, opacity: 0 }}
                      animate={{ scale: 1, opacity: 1 }}
                      exit={{ scale: 0.8, opacity: 0 }}
                      className="bg-[#FCFDFD] border border-slate-100 rounded-2xl p-6 w-full max-w-sm text-center shadow-lg"
                  >
                      <h3 className="text-xl text-gray-600 font-bold ">Incoming Call</h3>
                      <p className="text-gray-600 mt-2">Would you like to accept the call?</p>
                      <div className="flex gap-4 justify-center mt-6">
                          <button onClick={handleAcceptCall} className="px-6 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 transition active:scale-95 cursor-pointer">Accept</button>
                          <button onClick={handleDeclineCall} className="px-6 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 transition active:scale-95 cursor-pointer">Decline</button>
                      </div>
                  </motion.div>
              </motion.div>
          </AnimatePresence>
    ...
    return (
      <>
        ...
        <div className="h-screen w-screen bg-[#FCFDFD] text-white flex flex-col md:flex-row overflow-hidden">
          {/* Main Video Area */}
          <div className="flex-1 flex flex-col relative w-full h-full md:h-screen">
            <header className="absolute top-4 left-0 w-full px-4 z-10 flex justify-between items-center">
                <div className="flex items-center gap-2 text-white bg-black/30 backdrop-blur-sm px-3 py-1.5 rounded-lg">
                  ...
                  <span className="font-semibold">Calmnest</span>
                </div>
            ...
            <div className="flex-1 w-full h-full bg-[#FCFDFD] flex items-center justify-center relative">
              {remoteStream ? (
                <>
                  <ReactPlayer 
                    playing 
